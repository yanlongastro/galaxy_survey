...
class ps_interpolation:
    """
    We now discontinue this routine, since we can run camb and extract the oscillation part easily.
        ...
        if not ap_effect:
            for key in self.additional_parameters:
                ...
                if scipy_interp:
                    ...
                    n = len(pdata)
                    dpdp = lambda x, mu=0, z=0: two_value_interpolation_c(pdata[:,0], pdata[:,1], x, n)
...
class survey:
    ...
    todos:  - add FOG
    """
    def __init__(self, cosmo, ps, survey_geometrics, survey_parameters, ingredients, initial_params, a_imax=4, b_imax=5):
        self.cosmo = cosmo
    ...
    #def get_ready(self):
        ...
        ndim_p = 2+len(self.additional_parameters)
        ndim_b = 2+len(self.additional_parameters)
        if 'bias_in_fisher' in self.ingredients:
            ndim_b += 3
            ndim_p += 1
        if 'polynomial_in_fisher' in self.ingredients:
            # ndim_b += a_imax+b_imax+1
            ndim_p += a_imax+b_imax+1
    ...
    def power_spectrum(self, double k, mu=0.0, z=0.0, nw=False, linear=False, noise=False, external_biases=False, biases=(1., -1., 0.)):
        cdef double p = self.ps.matter_power_spectrum_no_wiggle(k)
        if nw==False:
            p *= 1 + self.oscillation_part(k, mu, z)
        if linear==False:
            p *= pow(self.rsd_factor_z1(z, mu, external_biases=external_biases, biases=biases), 2)
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def power_spectrum_derivative_bias(self, double k, mu=0., z=0., nw=False, noise=False):
        """
        ...
        for i in range(len(dPdb)):
            ...
            Pm = self.power_spectrum(k, mu=mu, z=z, nw=nw, external_biases=True, biases=tuple(biases_temp))

            biases_temp = biases
            biases_temp[i] += dbiases[i]
            Pp = self.power_spectrum(k, mu=mu, z=z, nw=nw, external_biases=True, biases=tuple(biases_temp))
    ...
    def power_spectrum_derivative_polynomial(self, double k, mu=0.0, z=0.0, linear=False, a_imax=4, b_imax=5):
        if not('polynomial_in_fisher' in self.ingredients):
            return []
        
        o = self.oscillation_part(k, mu, z)
        p = self.ps.matter_power_spectrum_no_wiggle(k)
        do = 1+o
        dp = p*(1+o)
        if linear==False:
            rsd = pow(self.rsd_factor_z1(z, mu), 2)
            dp *= rsd
            do *= rsd
        grow = pow(self.cosmo.linear_growth_factor(z)/self.cosmo.D0, 2)
        dp *= grow
        do *= grow

        dps = []
        for n in range(0, a_imax+1):
            dps.append(do*pow(k, n))
        for m in range(1, b_imax+1):
            dps.append(dp*pow(k, m*2))
        return np.array(dps)
    ...
    def integrand_ps(self, k, mu, z, simplify=False, noise=True):
        ...
        dp_bias = self.power_spectrum_derivative_bias(k, mu=mu, z=z, noise=False)
        dp_additional = self.power_spectrum_derivative_additional(k, mu=mu, z=z)
        dp_polynomial = self.power_spectrum_derivative_polynomial(k, mu=mu, z=z, a_imax=self.a_imax, b_imax=self.b_imax)
        dp = np.concatenate((dp, dp_additional, dp_bias, dp_polynomial))
    ...
    def fisher_matrix_ps(self, regions, addprior=False, tol=1e-4, rtol=1e-4, div_k=0, div_mu=0, parallel=False):
        """
        ...
        """
        fisher_ps_list = np.zeros((len(self.zmid_list), *(self.dp_shape)))
        ...
        for iz in range(len(self.zmid_list)):
            ...
            entries = ['alpha', 'beta']
            for k in self.additional_parameters:
                entries.append(k)
            if 'bias_in_fisher' in self.ingredients:
                entries.append('bias_b1-%s'%bin_label)
            if 'polynomial_in_fisher' in self.ingredients:
                for n in range(self.a_imax+1):
                    entries.append('poly_a%d-%s'%(n, bin_label))
                for n in range(1, self.b_imax+1):
        ...
        nzs = len(self.fisher_ps_list)
        if 'bias_in_fisher' in self.ingredients:
            nbp = 1
        else:
            nbp = 0
        if 'polynomial_in_fisher' in self.ingredients:
            nbp += self.a_imax+self.b_imax+1
        fisher_ps = np.zeros((2+nap+nbp*nzs, 2+nap+nbp*nzs))
        fisher_ps[:2+nap, :2+nap] = np.sum(fisher_ps_list, axis=0)[:2+nap, :2+nap]
        for i in range(nzs):
            fisher_ps[2+nap+i*nbp:2+nap+(i+1)*nbp, 2+nap+i*nbp:2+nap+(i+1)*nbp] = fisher_ps_list[i][2+nap:, 2+nap:]
            fisher_ps[2+nap+i*nbp:2+nap+(i+1)*nbp, :2+nap] = fisher_ps_list[i][2+nap:, :2+nap]
            fisher_ps[:2+nap, 2+nap+i*nbp:2+nap+(i+1)*nbp] = fisher_ps_list[i][:2+nap, 2+nap:]
        #print(fisher_bs)
        self.fisher_ps = fisher_ps

        fisher_ps_ = fisher_matrix_ps_list[0]
        for i in range(1, nzs):
            fisher_ps_ = fisher_ps_.merge(fisher_matrix_ps_list[i])
        self.fisher_ps_ = fisher_ps_
    ...
    def bispectrum_derivative_polynomial(self, (double, double, double) kargs, muargs=(0., 0., 0.), z=0, coordinate='cartesian', nw=False, noise=False):
        """
        will give different results depending on the coordinate
        """
        cdef double k_1, k_2, k_3, mu_1, mu_2, mu_3
    ...
    def fisher_matrix_bs(self, regions, method='sobol', addprior=False, tol=1e-4, rtol=1e-4, unique=True, verbose=False, k_max_bi=2333.):
        ...
        fisher_bs_ = fisher_matrix_bs_list[0]
        for i in range(1, nzs):
            fisher_bs_ = fisher_bs_.merge(fisher_matrix_bs_list[i])
        self.fisher_bs_ = fisher_bs_
